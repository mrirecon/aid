...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, temporal=False, x0=None
    ):
        """
        ...
        assert t.shape == (B,)
        if temporal:
            if "sampling" in model_kwargs and model_kwargs["sampling"]:
                input = th.cat([x0, x], dim=1)
            else:
                input = th.cat([x0, x], dim=-1)
        else:
            input = x
        model_output = model(input, self._scale_timesteps(t), **model_kwargs)

        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            if not temporal:
                expect_shape = (B, C * 2, *x.shape[2:])
            else:
                expect_shape = (B, T, C * 2, *x.shape[3:])

            assert model_output.shape == expect_shape
            model_output, model_var_values = th.split(model_output, C, dim=f_dim)
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, use_adpative_variance=False, model_kwargs=None):
        """
    ...
    def p_sample(
        ...
        if noisy_x0:
            assert temporal
            noise_x0 = th.randn_like(x0)
            x0 = self.q_sample(x0, t, noise=noise_x0)
        ...
        if cond_fn is not None:
            for _ in range(cond_fn_steps):
                x = self.condition_mean(cond_fn, out, x, t, model_kwargs=model_kwargs, use_adpative_variance=use_adpative_variance)
                out["mean"] = x
    ...
    def p_sample_temporal_loop(
        ...
        if extra_steps > 0, we will generate extra samples after seq_length
        if warm_start is True, we will use x0 to generate the next sample and shape[1] must be 1 or it always should be 1
        """
        ...
        while i < seq_length+extra_steps:
            ...
            iter_fn = partial(self.p_sample_loop_progressive, 
                model,
                shape,
                noise=noise,
                clip_denoised=clip_denoised,
    ...
    def _vb_terms_bpd(
        self, model, x_start, x_t, t, clip_denoised=True, model_kwargs=None, temporal=False, x0=None
    ):
        """
        ...
        out = self.p_mean_variance(
            model, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs, temporal=temporal, x0=x0
        )
        kl = normal_kl(
    ...
    def training_losses_temporal(self, model, input, t, model_kwargs=None, noise=None, noisy_x0=False):
        """
        Compute training losses for a single timestep.

        :param model: the model to evaluate loss on.
        :param input: [x0, x_start]
        :param t: a batch of timestep indices.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param noise: if specified, the specific Gaussian noise to try to remove.
        :return: a dict with the key "loss" containing a tensor of shape [N].
                 Some mean or variance settings may also have other keys.
        """
        x0, x_start = th.chunk(input, 2, dim=-1)
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(th.cat([x0, x_t], dim=-1), self._scale_timesteps(t), **model_kwargs)
            ...
            if self.model_var_type in [
                ...
                B, T, C = x_t.shape[:3]
                assert model_output.shape == (B, T, C * 2, *x_t.shape[3:])
