...
class KarrasDenoiser:
    ...
    def training_losses_temporal(self, model, input, sigmas, model_kwargs=None, noise=None, noisy_x0=False):
        ...
        if noisy_x0:
            x0 = x0 + th.randn_like(x0) * append_dims(sigmas, dims)
            
        model_output, denoised = self.denoise(model, x_t, sigmas, x0=x0, **model_kwargs)
    ...
    def denoise(self, model, x_t, sigmas, x0=None, temporal=False, **model_kwargs):
        ...
        if x0 is not None:
            if not temporal:
                ins = th.cat([x0, c_in * x_t], dim=-1)
            else:
                ins = th.cat([x0, c_in * x_t], dim=1)
        else:
            ins = c_in * x_t
        model_output = model(ins, rescaled_t, **model_kwargs)
...
def karras_sample(
    ...
    sample_fn = {
        "heun": sample_heun if not temporal else sample_heun_temporal,
        "dpm": sample_dpm,
    ...
    # this function will slow down the sampling process
    def denoiser(x_t, sigma, x0, temporal, model_kwargs):
        _, denoised = diffusion.denoise(model, x_t, sigma, x0, temporal, **model_kwargs)
        if clip_denoised:
...
def heun_func(x, sigmas, denoiser, generator, progress=False, callback=None, s_churn=0.0, s_tmin=0.0, s_tmax=float("inf"), s_noise=1.0, model_kwargs=None, x0=None, temporal=False):
    ...
    for i in indices:
        ...
        denoised = denoiser(x, sigma_hat * s_in, x0, temporal, model_kwargs)
        ...
        if sigmas[i + 1] == 0:
            ...
            # Heun's method
            x_2 = x + d * dt
            denoised_2 = denoiser(x_2, sigmas[i + 1] * s_in, x0, temporal, model_kwargs)
...
def sample_heun(
    ...
    return heun_func(x, sigmas, denoiser, generator, progress, callback, s_churn, s_tmin, s_tmax, s_noise, model_kwargs, x0)

@th.no_grad()
def sample_heun_temporal(
    denoiser,
    shape,
    sigmas,
    x0,
    ...
    while i < seq_length + extra_steps:
        ...
        if not warm_start and i < seq_length:
            model_kwargs['idx_t'] = i #TODO check this idx_t
        else:
        ...
        x_T = generator.randn(*shape, device=x0.device) * sigmas[0]
        new_elem = heun_func(x_T, sigmas, denoiser, generator, progress, callback, s_churn, s_tmin, s_tmax, s_noise, model_kwargs, x0, temporal=True)
