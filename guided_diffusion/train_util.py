...
def load_vqgan(config, ckpt_path=None, is_gumbel=False):
  ...
  if ckpt_path is not None:
    sd = th.load(ckpt_path, map_location="cpu")["state_dict"]
    missing, unexpected = model.load_state_dict(sd, strict=False)
  return model.eval()

def get_vqmodel(config_path, ckpt_path, is_gumbel=False):
  config = load_config(config_path, display=False)
  model = load_vqgan(config, ckpt_path, is_gumbel)
  return model
...
class TrainLoop:
        ...
        *,
        ...
        if self.latent:
            if vae:
                ...
                self.model = AutoencoderKL.from_pretrained(f"stabilityai/sd-vae-ft-ema").to(dist_util.dev()) # latent_size = image_size // 8
            else:
                self.model = get_vqmodel(vq_config, vq_ckpt).to(dist_util.dev())    # latent_size = image_size // 4
            @th.no_grad()
            def encode(x, scale=self.scale):
                if vae:
                    return self.model.encode(x).latent_dist.sample().mul_(scale)
                else:
    ...
    def run_loop(self, temporal=False, noisy_x0=False, multistage=0):
        while (
    ...
    def run_step(self, batch, cond, temporal, noisy_x0, multistage):
    ...
    def forward_backward(self, batch, cond, temporal, noisy_x0, multistage):
